  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 flex px-6 md:px-10 py-3 flex-row justify-between items-center z-50 w-full transition-all duration-500 ${
      ...
      <div className="flex items-center space-x-3 group cursor-pointer">
        <div className="h-10 w-10 bg-gold rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
           <svg
            stroke="currentColor"
            fill="black"
            strokeWidth="0"
      ...
      <div className="hidden md:block">
        <div className="flex items-center space-x-6 text-white/90 font-medium tracking-wide text-sm">
          <Link
            href="#about"
            className="hover:text-gold duration-300 transition-colors relative group"
          >
            About
      ...
      <div className="flex items-center gap-4">
        <Link href="https://github.com/pratik-71" target="_blank" rel="noopener noreferrer">
          <button className="hidden sm:block px-5 py-2 bg-gold text-black font-bold rounded-[2.5rem] hover:scale-105 active:scale-95 transition-all duration-300 text-sm">
            GITHUB
          </button>
        </Link>
        <button 
      ...
      {/* Mobile Nav */}
      <div className={`fixed inset-0 bg-black/95 backdrop-blur-2xl z-[100] transition-all duration-500 flex flex-col items-center justify-center gap-8 ${opennav ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <button 
        ...
        {['About', 'Skills', 'Experience', 'Project', 'Education', 'Certificates', 'Contact'].map((item) => (
          <Link
            ...
            className="text-3xl font-black tracking-tighter text-white hover:text-gold transition-all duration-300"
            onClick={() => setopennav(false)}
          >
            {item.toUpperCase()}
          </Link>
